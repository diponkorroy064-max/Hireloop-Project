...
const RecruiterJobs = async () => {
    ...
    const jobs = await getCompanyJobs(company._id, "active") || [];

    console.log("Company data in RecruiterJobs:", company);
                        ...
                        <Table.Body emptyContent={"No jobs found for this company."}>
                            {jobs.map((job) => (
                                <Table.Row key={job._id?.$oid || job._id}>
                                    {/* Job Title */}
                                    <Table.Cell>
                                        <div className="font-medium text-default-800 text-white">
                                            {job.jobTitle}
                                        </div>
                                    </Table.Cell>
                                    ...
                                    {/* Location */}
                                    <Table.Cell>
                                        <span className="text-sm text-default-600 text-white">
                                    ...
                                    <Table.Cell>
                                        <div className="relative flex items-center gap-2">
                                            <Tooltip content="Video Details">
                                                ...
                                                >
                                                    <Eye className="text-default-400 w-4 h-4 text-white" />
                                                </Button>
                                            </Tooltip>
                                            <Tooltip content="Edit Job">
                                                <Button
                                                    ...
                                                    aria-label="Edit job"
                                                >
                                                    <Edit2 className="text-default-400 w-4 h-4 text-white" />
                                                </Button>
                                            </Tooltip>
